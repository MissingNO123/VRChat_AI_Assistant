...
class LogWatcher:
    ...
    def _parse_log_location(self, line):
        ...
        if '[Behaviour] Joining wrld_' in line:
            ...
            world_re = r'wrld_([\w-]+):(\d+)'
            match_world = re.match(world_re, self.full_world_id)
            if not match_world:
                return
            ...
            inst_tag_re = r'~(\w+)(?:\(([\w-]+)\))?'
            tags = {}
            match_tag = re.finditer(inst_tag_re, self.full_world_id)
            ...
            instance_privacy_map = {
                'public': 'Public',
                'hidden': 'Friends and Friends of Guests',
            ...
            group_privacy_map = {
                ...
                'plus': 'Group Members and Friends', 
                'groupPlus': 'Group Members and Friends', 
                'public': 'Group-owned Public', 
            }
    ...
    def _parse_log_on_player_joined_or_left(self, line):
        if  (
            ...
            if substring_start >= len(line):
                return
            start_parenth_pos = line.find(' (', substring_start)
            if start_parenth_pos > 0:
                end_parenth_pos = line.find(')', start_parenth_pos)
                user_display_name = line[substring_start:start_parenth_pos]
                user_id = line[start_parenth_pos+1:end_parenth_pos]
