...
class ManualTextEntryWindow(customtkinter.CTkToplevel):
    def __init__(self, master, title):
        ...
        self.result = None
        self.should_attach_screenshot = False
        ...
        self.textfield_text_entry.bind("<Control-Return>", self._start_send_screenshot)
        self.textfield_text_entry.bind("<Return>", self._start_send)
        self.textfield_text_entry.bind("<KP_Enter>", self._start_send)
        self.textfield_text_entry.bind("<Control-a>", lambda e: self.textfield_text_entry.select_range(0, "end"))
        self.textfield_text_entry.bind("<Control-r>", self._retry_generate)
        self.textfield_text_entry.bind("<Control-p>", self._reset_status_flags)
                    ...
                    if type(message["content"]) == list:
                        for item in message["content"]:
                            if item.get("type") == "text":
                                str_array.append( f'{"User" if message["role"] == "user" else "AI"}: {item["text"]}' )
                            elif item.get("type") == "image_url":
                                str_array.append( f'{"User" if message["role"] == "user" else "AI"}: [Image]' )
                    else: 
    ...
    def _start_send(self, event=None):
        # Wait if we're already generating
        ...
        # Append user message (or image) to chat history
        if self.should_attach_screenshot:
            self.should_attach_screenshot = False
            screenshot = eyes.get_vrchat_screenshot()
            if screenshot is not None:
                image_as_b64 = eyes.convert_to_b64(screenshot)
                funcs.append_image_message(image_as_b64, user_text)
                self.addtext(f"\n---\nUser:{user_text}\n[Image]")
                with open("screenshot.png", "wb") as f:
                    f.write(eyes.convert_to_png(screenshot))
            else:
                self.addtext("\n---\nUser: [Failed to attach image]")
        else:
    ...
    def _start_send_screenshot(self, event=None):
        user_text = self.text_entry.get().strip()
        if len(user_text) <= 0: return
        print("Attaching Screenshot...")
        self.should_attach_screenshot = True
        self._start_send()
    ...
    def _generate(self, user_text="", *args, **kwargs):
        ...
        try:
            ...
            if is_function_call:
                funcs.v_print(f'\n--AI Function call took {end_time - start_time:.3f}s')
                ...
                self.result = funcs.clear_between_tags(self.result).strip()
                if len(self.result):
                    funcs.append_bot_message(self.result)
            funcs.v_print(f'\n--AI API took {end_time - start_time:.3f}s')
            print()
    ...
    def _retry_generate(self, event=None):
        if opts.generating: return
        funcs.v_print("Retrying last message...")
        text = opts.message_array[-1]["content"]
        if type(text) == list:
        ...
        if opts.parrot_mode:
            ...
            generate_thread = threading.Thread(target=self._generate)
            generate_thread.start()
    
    def _reset_status_flags(self, event=None):
        print("Resetting status flags...")
        status_messages = []
        if opts.panic:
            opts.panic = False
            status_messages.append("panic reset.")
        if opts.generating:
            opts.generating = False
            status_messages.append("generating reset.")
        if opts.speaking:
            opts.speaking = False
            status_messages.append("speaking reset.")
        # if opts.is_speaking_lock.locked():
        #     opts.is_speaking_lock.release()
        #     status_messages.append("is_speaking_lock released.")
        if opts.trigger:
            opts.trigger = False
            status_messages.append("trigger reset.")
        if opts.bot_responded == False:
            opts.bot_responded = True
            status_messages.append("bot_responded reset.")
        self.textfield_text_entry.configure(state="normal")
        self.button_send.configure(text="Send", state="normal")
        if len(status_messages) > 0:
            print(f"{' '.join(status_messages)}")
            vrc.chatbox(f"⚠ {' '.join(status_messages)}")
...
class AIStuffFrame(customtkinter.CTkFrame):
    def __init__(self, master, title):
        ...
        self.selected_whisper_model = customtkinter.StringVar(value=opts.whisper_model_size)
    ...
    def _set_whisper_model(self, choice):
        opts.whisper_model_size = choice
...
class AudioStuffFrame(customtkinter.CTkFrame):
    def __init__(self, master, title):
        ...
        row_id += 1
        self.spinbox_rms_threshold = FloatSpinbox(self, step_size=0.25, min=0, max=16384, value=self.rms_threshold.get(), command=self._spinbox_callback)
    ...
    def _update_audio_variables(self, event=None):
        try:
            opts.recording_threshold = float(self.rms_threshold.get())
    ...
    def _set_tts_engine(self, choice):
        match choice:
            ...
            case "TikTok":
                ...
                self.frame_tts_options.grid(row=2, column=0, sticky="nsew", padx=0, pady=0)
            case "AllTalkTTS":
                opts.tts_engine = texttospeech.AllTalkTTS()
                if not opts.tts_engine.ready:
                    print("AllTalk TTS engine is not ready!")
                    popup = Popup(self, window_title="TTS engine not ready",
                                  window_text="Could not connect to AllTalk!\nMake sure the server is running.", 
                                  button_text="OK")
                    popup.after(250, popup.focus) # Why do I need to wait for this???
                self.frame_tts_options = AllTalkTTSOptionsFrame(master=self, title="AllTalk TTS Configuration")
                self.frame_tts_options.grid(row=2, column=0, sticky="nsew", padx=0, pady=0)
...
class AllTalkTTSOptionsFrame(customtkinter.CTkFrame):
    def __init__(self, master, title):
        super().__init__(master, fg_color="transparent")
        self.title = title
        self.grid_columnconfigure(0, weight=1)
        ...
        self.voices = []
        self.rvc_voices = []
        self.rvc_pitch = customtkinter.StringVar(value="0")
        self.speaking_rate = customtkinter.StringVar(value="1.0")

        if not isinstance(opts.tts_engine, texttospeech.AllTalkTTS):
            self.label_error = customtkinter.CTkLabel(self, text="AllTalk is not the currently selected TTS engine.", fg_color="transparent")
            self.label_error.grid(row=1, column=0, sticky="ew", pady=(4,1), padx=5)
            return
        ...
        self.selected_alltalk_tts_voice_name = customtkinter.StringVar(value=opts.alltalk_selected_voice)

        for rvcvoice in opts.tts_engine.rvc_voices:
            self.rvc_voices.append(rvcvoice)

        self.selected_alltalk_tts_rvc_voice_name = customtkinter.StringVar(value=opts.alltalk_selected_rvc_voice)
        
        row_id = 1

        # voices selection
        self.label_alltalk_tts_voice = customtkinter.CTkLabel(self, text="Voice Name: ", fg_color="transparent")
        self.label_alltalk_tts_voice.grid(row=row_id, column=0, sticky="w", pady=(4,1), padx=5)
        row_id += 1

        self.dropdown_alltalk_tts_voice_select = customtkinter.CTkOptionMenu(self, dynamic_resizing=False, variable=self.selected_alltalk_tts_voice_name, values=self.voices, command=self._set_tts_voice)
        self.dropdown_alltalk_tts_voice_select.grid(row=row_id, column=0, sticky="ew", padx=10, pady=(0,10))
        row_id += 1

        self.label_alltalk_tts_rvc_voice = customtkinter.CTkLabel(self, text="RVC Voice Name: ", fg_color="transparent")
        self.label_alltalk_tts_rvc_voice.grid(row=row_id, column=0, sticky="w", pady=(4,1), padx=5)
        row_id += 1

        self.dropdown_alltalk_tts_rvc_voice_select = customtkinter.CTkOptionMenu(self, dynamic_resizing=False, variable=self.selected_alltalk_tts_rvc_voice_name, values=self.rvc_voices, command=self._set_tts_rvc_voice)
        self.dropdown_alltalk_tts_rvc_voice_select.grid(row=row_id, column=0, sticky="ew", padx=10, pady=(0,10))
        row_id += 1

        #rvc pitch, and speaking rate
        self.label_rvc_pitch = customtkinter.CTkLabel(self, text="RVC Pitch: ", fg_color="transparent")
        self.label_rvc_pitch.grid(row=row_id, column=0, sticky="w", pady=(4,1), padx=5)
        row_id += 1
        self.spinbox_rvc_pitch = IntSpinbox(self, step_size=1, min=-12, max=12, value=self.rvc_pitch.get(), command=self._spinbox_callback)
        self.spinbox_rvc_pitch.grid(row=row_id, column=0, columnspan=2, sticky="ew", pady=2, padx=10)
        row_id += 1

        self.label_speed = customtkinter.CTkLabel(self, text="Speaking Rate: ", fg_color="transparent")
        self.label_speed.grid(row=row_id, column=0, sticky="w", pady=(4,1), padx=5)
        row_id += 1
        self.spinbox_speed = FloatSpinbox(self, step_size=0.1, min=0.5, max=2.0, value=self.speaking_rate.get(), command=self._spinbox_callback)
        self.spinbox_speed.grid(row=row_id, column=0, columnspan=2, sticky="ew", pady=2, padx=10)
        row_id += 1

    def _set_tts_voice(self, choice):
        if not isinstance(opts.tts_engine, texttospeech.AllTalkTTS):
            print("AllTalk is not the currently selected TTS engine")
            return
        opts.tts_engine.set_voice(choice)
        opts.alltalk_selected_voice = choice
        print(f'Set AllTalk voice to {choice}')

    def _set_tts_rvc_voice(self, choice):
        if not isinstance(opts.tts_engine, texttospeech.AllTalkTTS):
            print("AllTalk is not the currently selected TTS engine")
            return
        opts.tts_engine.set_rvc_voice(choice)
        opts.alltalk_selected_rvc_voice = choice
        print(f'Set AllTalk RVC voice to {choice}')

    def _spinbox_callback(self):
        self.rvc_pitch.set( self.spinbox_rvc_pitch.get() )
        self.speaking_rate.set( self.spinbox_speed.get() )
    
    def _update_rvc_pitch(self):
        if not isinstance(opts.tts_engine, texttospeech.AllTalkTTS):
            print("AllTalk is not the currently selected TTS engine")
            return
        opts.tts_engine.set_rvc_pitch(self.spinbox_rvc_pitch.get())
        print(f'Set AllTalk RVC pitch to {self.spinbox_rvc_pitch.get()}')
    
    def _update_speed(self):
        if not isinstance(opts.tts_engine, texttospeech.AllTalkTTS):
            print("AllTalk is not the currently selected TTS engine")
            return
        spinbox_value = self.spinbox_speed.get()
        #convert to str so i can round to 3 decimal places, then back to float
        spinbox_value_rounded = float(str(spinbox_value)[:5])
        opts.tts_engine.set_speed(spinbox_value_rounded)
        self.spinbox_speed.set(spinbox_value_rounded)
        print(f'Set AllTalk speed to {spinbox_value_rounded}')
